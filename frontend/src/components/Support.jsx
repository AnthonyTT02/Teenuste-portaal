    ...
    <div className="w-full max-w-5xl animate-fade-in-up">
      <div className="backdrop-blur-xl bg-white/60 border border-white/40 shadow-2xl rounded-[2rem] p-10 mt-8 relative">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#4f46e5] to-[#3b82f6] mb-2 tracking-tight">Support Dashboard</h1>
            <p className="text-gray-500 font-medium">{openTickets.length} open · {resolvedTickets.length} resolved</p>
          </div>
        ...
        <div className="flex gap-2 mb-6">
          {['open', 'resolved'].map(t => (
            <button key={t} onClick={() => setTab(t)} className={`px-5 py-2 rounded-xl font-bold text-sm transition-colors capitalize ${tab === t ? 'bg-brand text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              {t} ({t === 'open' ? openTickets.length : resolvedTickets.length})
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <p className="text-gray-500 text-lg font-medium">{tab === 'open' ? 'No open tickets!' : 'No resolved tickets yet'}</p>
          </div>
        ) : (
            ...
            {displayTickets.map(t => (
              <div key={t.id} className="bg-white/70 border border-white/60 rounded-2xl p-5 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-bold text-gray-800">Ticket #{t.id}</h3>
                    <p className="text-sm text-gray-500">Worker: <span className="font-semibold text-gray-700">{t.order?.worker_user ? `${t.order.worker_user.government_name} ${t.order.worker_user.government_surname}` : 'Not assigned'}</span> · Order #{t.order_id}</p>
                    {t.user?.phone && <p className="text-xs text-gray-400">Customer Phone: {t.user.phone}</p>}
                  </div>
